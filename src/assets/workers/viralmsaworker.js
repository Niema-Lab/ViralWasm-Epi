...
const init = async () => {
	...
	// load in ViralMSA.py
	pyodide.FS.writeFile(PATH_TO_PYODIDE_ROOT + 'ViralMSA.py', await (await fetch(VIRAL_MSA_LINK)).text(), { encoding: "utf8" });

	// load in ViralMSAWeb.py
	ViralMSAWeb = await (await fetch(VIRAL_MSA_WEB_LINK)).text()
...
const runViralMSA = async (inputSequences, referenceSequence, refID, omitRef) => {
	...
	if (refID) {
		...
		if (!pyodide.FS.readdir(`${PATH_TO_PYODIDE_ROOT}/cache/`).includes(refID)) {
			...
			const referenceSequence = await (await fetch(VIRAL_MSA_REF_GENOMES_DIR + refID + "/" + refID + ".fas")).text();
			const refIndex = new Uint8Array(await (await fetch(VIRAL_MSA_REF_GENOMES_DIR + refID + "/" + refID + ".fas.mmi")).arrayBuffer());
